...
At the end of either the prepare nodes or deploy nodes steps that have failed
due to timeout, dependency failure, or success criteria failure and are marked
as critical, an Airflow Exception should be raised to trigger a failure in the
mainline workflow being executed.
