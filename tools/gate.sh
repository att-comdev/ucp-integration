...
MANIFEST_ARG=${1:-multinode-deploy}
GATE_MANIFEST=${WORKSPACE}/tools/ucp_gate/manifests/${MANIFEST_ARG}.json
...
STAGES_DIR=${WORKSPACE}/tools/ucp_gate/stages
...
while read -u 3 stage; do
    ...
    if echo "${stage}" | jq -r '.arguments | @sh' | xargs "${STAGE_CMD}" ; then
        ...
        if echo "${stage}" | jq -e .on_error > /dev/null; then
            ...
            ON_ERROR=${WORKSPACE}/tools/ucp_gate/on_error/$(echo "${stage}" | jq -r .on_error)
            set +e
            $ON_ERROR
        fi
