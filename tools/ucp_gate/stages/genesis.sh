...
cp /root/ucp/ucp_drydock_kvm_ssh_key.yaml /root/deploy/ucp-integration/site/gate-multinode/secrets/passphrases/ucp_drydock_kvm_ssh_key.yaml
...
# Add a space in front so that we can append the content to ucp_drydock_kvm_ssh_key.yaml
...
set -o pipefail
ssh_cmd "${GENESIS_NAME}" /root/promenade/setup_genesis.sh 2>&1 | tee -a "${LOG_FILE}"
set +o pipefail
