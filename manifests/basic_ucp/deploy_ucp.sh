...
export AIRFLOW_IMAGE=${AIRFLOW_IMAGE:-"docker.io/eanylin/airflow-helm:1.8.1-1.6.8-v0.6.0"}
...
while [[ -z $(kubectl get pods -n ucp | grep deckhand | grep Running) ]]
do
  sleep 5
done

while [[ -z $(kubectl get pods -n ucp | grep shipyard | grep Running) ]]
do
  sleep 5
done
