...
function run_pegleg_collect() {
  ...
  IMAGE=${PEGLEG_IMAGE}
  ${PEGLEG} site -p /workspace/ucp-integration/deployment_files collect ${TARGET_SITE} -s /workspace/collected
}
...
function generate_certs() {
  ...
  cp ${WORKSPACE}/genesis/certificates.yaml ${WORKSPACE}/ucp-integration/deployment_files/${TARGET_SITE}/secrets
}
...
function genesis_complete() {
  ...
  echo ""
  echo "Shipyard executable script can be found at ${WORKSPACE}/shipyard/tools/shipyard_image_run.sh"
}
